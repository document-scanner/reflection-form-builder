...
public class AmountMoneyComponent extends javax.swing.JPanel {
     ...
     * @throws AmountMoneyCurrencyStorageException if such an exception occurs
     */
    @SuppressWarnings("PMD.AvoidInstantiatingObjectsInLoops")
    public AmountMoneyComponent(AmountMoneyCurrencyStorage amountMoneyCurrencyStorage,
            AmountMoneyExchangeRateRetriever amountMoneyExchangeRateRetriever,
            MessageHandler messageHandler) throws AmountMoneyExchangeRateRetrieverException,
        ...
        this.amountIntegerSpinner.addChangeListener((ChangeEvent e) -> {
            for(AmountMoneyComponentUpdateListener updateListener : updateListeners) {
                updateListener.onUpdate(new AmountMoneyComponentUpdateEvent(getValue()));
            }
        });
        this.amountDecimalSpinner.addChangeListener((ChangeEvent e) -> {
            for(AmountMoneyComponentUpdateListener updateListener : updateListeners) {
                updateListener.onUpdate(new AmountMoneyComponentUpdateEvent(getValue()));
            }
        });
        this.currencyComboBox.addItemListener((ItemEvent itemEvent) -> {
            ...
            amountDecimalSpinner.setValue(bdIntValue%100);
            if(bd.doubleValue() > newAmount*100) {
                //rounding up has occured
                amountIntegerSpinner.setValue((int)newAmount+1);
            }else {
                amountIntegerSpinner.setValue((int)newAmount);
            }
    ...
    private double getAmount() {
        return ((Number)amountIntegerSpinner.getValue()).doubleValue()
