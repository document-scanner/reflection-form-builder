...
public abstract class CachedOnlineAmountMoneyExchangeRateRetriever extends OnlineAmountMoneyExchangeRateRetriever {
    ...
    public Pair<Map<Currency, Double>, Currency> getResult() throws AmountMoneyExchangeRateRetrieverException {
        ...
        if(result == null) {
            if(!fileCacheFile.exists()) {
                ...
                try {
                    xStream.toXML(new ImmutablePair<>(timestampNew, result),
                            Files.newOutputStream(fileCacheFile.toPath()));
     ...
     * @return the retrieved exchange rate data
     ...
     * @throws AmountMoneyExchangeRateRetrievalException if an exception related
     *     to a network failure or missing network connection occured
     */
     ...
     * @return the fetched conversion rate
     * @throws AmountMoneyExchangeRateRetrieverException if an exception
     *     unrelated to a network failure or missing network connection occured
     */
    @Override
    public double fetchConversionRate(Currency currency) throws AmountMoneyExchangeRateRetrieverException {
        Pair<Map<Currency, Double>, Currency> result0 = getResult();
        assert result0 != null;
        if(Currency.getReferenceCurrency() == null) {
            Currency.setReferenceCurrency(result0.getValue());
        }else {
            if(!Currency.getReferenceCurrency().equals(result0.getValue())) {
        ...
        Double retValue = result0.getKey().get(currency);
        if(retValue == null) {
