...
public class XMLStorage<T extends Identifiable> extends AbstractStorage<T, XMLStorageConf> implements Storage<T, XMLStorageConf> {
    ...
    public void store(T object) throws StorageException {
        ...
        try {
            existingObjects = (List<Object>) xStream.fromXML(Files.newInputStream(file.toPath()));
        ...
        xStream = new XStream();
        try {
            xStream.toXML(existingObjects, Files.newOutputStream(file.toPath()));
        } catch (IOException ex) {
            throw new StorageException(ex);
        }
    }
    ...
    public <U extends T> U retrieve(Object id, Class<U> clazz) throws StorageException {
        ...
        try {
            existingObjects = (List<U>) xStream.fromXML(Files.newInputStream(file.toPath()));
        } catch (IOException ex) {
            throw new StorageException(ex);
        }
