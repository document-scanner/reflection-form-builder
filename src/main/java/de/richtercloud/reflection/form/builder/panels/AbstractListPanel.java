 ...
 * Provides buttons to add a new item and remove the selected intervals(s).
 * Provides a buttons to move the selected items up and down the list (if
 * multiple items are selected they're moved as if they were the only selected
 * one by one). The up button does nothing for the topmost item if it is
 * selected as does the down button for the bottommost item. The edit button
 * starts editing the item; implementation is up to subclasses (in
 * {@link #editRow() }). The "Select all" and "Invert selection" buttons select
 * all items and invert the selection of items respectively.
 *
 * @author richter
...
public abstract class AbstractListPanel<T, L extends ListPanelItemListener<T>, M extends ListPanelTableModel<T>, R extends ReflectionFormBuilder> extends javax.swing.JPanel {
    ...
    public final static Comparator<Integer> DESCENDING_ORDER = (Integer o1, Integer o2) -> o2.compareTo(o1);
    ...
    */
     ...
     * @param mainListCellEditor the main list cell editor
     * @param mainListCellRenderer the main list cell renderer
     * @param mainListModel the main list model
        ...
        for (L listener : this.getItemListeners()) {
            try {
                listener.onItemAdded(new ListPanelItemEvent<>(ListPanelItemEvent.EVENT_TYPE_ADDED,
                        getMainListModel().getRowCount(),
                        getMainListModel().getData()));
