...
public class MappingFieldHandler<T, E extends FieldUpdateEvent<T>, R extends ReflectionFormBuilder, C extends Component> implements FieldHandler<T, E, R, C> {
    ...
    public MappingFieldHandler(Map<Type, FieldHandler<?, ?, ?, ?>> classMapping,
        ...
        validateMapping(fieldAnnotationMapping, "fieldAnnotationMapping");
        validateMapping(classAnnotationMapping, "classAnnotationMapping");
    ...
    public Pair<JComponent, ComponentResettable> handle0(final Field field,
        ...
        for (Pair<Class<? extends Annotation>, FieldAnnotationHandler> pair : fieldAnnotationMapping) {
            if (field.getAnnotation(pair.getKey()) != null) {
                FieldAnnotationHandler fieldAnnotationHandler = pair.getValue();
                retValue = fieldAnnotationHandler.handle(field, //field
                        instance,
                ...
                return new ImmutablePair<JComponent, ComponentResettable>(retValue, fieldAnnotationHandler);
            }
        }
        // class annotations
        for (Pair<Class<? extends Annotation>, ClassAnnotationHandler<Object, FieldUpdateEvent<Object>, ?>> pair : classAnnotationMapping) {
            if (field.getType().getAnnotation(pair.getKey()) != null) {
                try {
                    ClassAnnotationHandler classAnnotationHandler = pair.getValue();
                    retValue = classAnnotationHandler.handle(field.getType(),
                            field.get(instance), //initialValue
                            new FieldUpdateListener() {
                                @Override
                                public void onUpdate(FieldUpdateEvent event) {
                                    try {
                            ...
                            reflectionFormBuilder);
                    return new ImmutablePair<JComponent, ComponentResettable>(retValue, classAnnotationHandler);
                } catch (Exception ex) {
                    throw new RuntimeException(ex);
                }
            }
